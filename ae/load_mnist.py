...
def load_mnist_data(threshold, fraction=1.0, examples_per_class=500,
    ...
    Returns:
        train_features - (torch.Tensor) An Nxd array of features, where N is
            the number of examples and d is the number of features.
        test_features - (torch.Tensor) An Nxd array of features, where M is the
            number of examples and d is the number of features.
        train_targets - (torch.Tensor) A 1D array of targets of size N.
        test_targets - (torch.Tensor) A 1D array of targets of size M.

    Taked from NUCS349 Neural Networks HW
    """
    ...
    train_features = torch.Tensor(train_features).reshape(-1, 1, 28, 28)
    test_features = torch.Tensor(test_features).reshape(-1, 1, 28, 28)
